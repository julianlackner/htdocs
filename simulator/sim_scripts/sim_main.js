...
function showModal(editing = false, id = null) {
    ...
    Simulation.editingElementId = editing ? id : null;

    if (editing) {
        const element = Simulation.elements[id];
        document.getElementById("elementType").value = element.type;
        document.getElementById("elementName").value = id;
...
function updateDropdowns() {
    ...
    for (const [name, element] of Object.entries(Simulation.elements)) {
        if (element.type === "tank") {
            pumpSourceDropdown.innerHTML += `<option value="${name}">${name}</option>`;
            pumpTargetDropdown.innerHTML += `<option value="${name}">${name}</option>`;
            levelDisplayTankDropdown.innerHTML += `<option value="${name}">${name}</option>`;
...
function addElement() {
    const type = document.getElementById("elementType").value;
    const name = document.getElementById("elementName").value || `Element_${Object.keys(Simulation.elements).length + 1}`;

    if (Simulation.editingElementId) {
        deleteElement(Simulation.editingElementId);
    }
    ...
    if (type === "tank") {
        ...
        Simulation.elements[name] = { type, capacity, level, alarmMax, alarmMin, initialLevel: level };
    } else if (type === "pump") {
        const flow = parseFloat(document.getElementById("pumpFlow").value) || 1000;
        const source = document.getElementById("pumpSource").value;
        const target = document.getElementById("pumpTarget").value;
        ...
        Simulation.elements[name] = { type, flow, source, target, running: false };
    } else if (type === "flowMeter") {
        const pump = document.getElementById("flowMeterPump").value;
        content += `<p>Pumpe: ${pump || "-"}</p>`;
        Simulation.elements[name] = { type, pump };
    }
...
function deleteElement(id) {
    const elementDiv = document.getElementById(id);
    if (elementDiv) elementDiv.remove();
    delete Simulation.elements[id];
}

function toggleLock(id) {
    // Prüfen, ob das Element in "Simulation.elements" oder "elements" existiert
    const element = Simulation?.elements?.[id] || elements?.[id];
    if (!element) {
        console.error(`Element mit ID "${id}" konnte nicht gefunden werden.`);
        return;
    }

    // Identifiziere das entsprechende Lock-Icon
    const lockIcon =
        document.querySelector(`#${id} .icon-container img[data-action="toggleLock"]`) ||
        document.querySelector(`#${id} .overlay img[alt="Lock"]`);

    if (!lockIcon) {
        console.error(`Lock-Icon für Element mit ID "${id}" konnte nicht gefunden werden.`);
        return;
    }

    // Aktualisiere den Zustand (locked/unlocked)
    element.locked = !element.locked;
    lockIcon.src = element.locked ? "images/icon_schloss_lock.png" : "images/icon_schloss_unlock.png";

    // Aktualisiere die Cursor- und Drag-Funktionalität basierend auf dem Zustand
    const elementDiv = document.getElementById(id);
    if (element.locked) {
        elementDiv.style.cursor = "default";
        elementDiv.onmousedown = null; // Entferne die Drag-Funktion
    } else {
        elementDiv.style.cursor = "move";
        makeDraggable(elementDiv); // Reaktiviere die Drag-Funktion
    }
}
